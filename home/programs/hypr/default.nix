  ...
  config = mkIf (cfg.gui.enable && cfg.hyprland.enable) {
    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        layerrule = [
          "match:namespace rofi, blur 1"
          "match:namespace launcher, blur 1"
          "match:namespace waybar, blur 1"
        ];

        windowrule = [
          "float 1, match:class kvantummanager"
          "float 1, match:class org.gnome.Settings"
          "float 1, match:class Lxappearance"
          "float 1, match:class obs"
          "float 1, match:class zathura"
          "float 1, match:class feh"
          "float 1, match:class qemu"
          "float 1, match:class biz.ntinfo.die"
          "float 1, match:class Ultimaker Cura"
          "float 1, match:class Pinentry-gtk-2"
          "float 1, match:title ^(wlroots)(.*)$"
          "float 1, match:class nm-connection-editor"
          "float 1, match:class org.kde.kdeconnect-settings"
          "float 1, match:class org.kde.kdeconnect.app"
          "float 1, match:class org.pulseaudio.pavucontrol"
          "float 1, match:class re.rizin.cutter, match:title ^(Open).*$"
          "float 1, match:class re.rizin.cutter, match:title ^Load Options$"
          "float 1, match:class vlc"
          "float 1, match:title Bluetooth Devices"
          "suppress_event fullscreen, match:class firefox, match:float 1"
          "match:class (pinentry-)(.*), stay_focused 1"
        ];
